...
const urls = [
  ...
  { url: "https://m.imdb.com/title/tt1520211/", id: "the_walking_dead" },
...
(async () => {
  ...
  for (let movies of urls) {
    ...
    await new Promise(async (resolve, reject) => {
      ...
      file.on("finish", () => {
        console.log(`Downloaded image for ${movies.id}`);
        resolve();
      });
      file.on("error", (error) => {
        reject(error);
      ...
      console.log(
        `${movies.id} encountered an error when downloading ${error}`
      );
    });
  }
  // Output JSON format
